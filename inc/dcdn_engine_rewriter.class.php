...
/**
 * DCDN_Engine_Rewriter
 *
 * @since 0.0.1
 */

class DCDN_Engine_Rewriter
{
    var $blog_url       = null;    // origin URL
    var $dcdn_url        = null;    // DCDN URL

    var $dirs           = null;    // included directories
    var $excludes       = []; // excludes
    var $relative       = false;   // use DCDN on relative paths
    var $https          = false;   // use DCDN on HTTPS
    ...
    protected function rewrite_url(&$asset) {
        ...
        // rewrite both http and https URLs if we ticked 'enable DCDN for HTTPS connections'
        if ($this->https) {
            $subst_urls = [
                'http:'.$blog_url,
                'https:'.$blog_url,
        ...
        if (strpos($asset[0], '//') === 0) {
            return str_replace($blog_url, $this->dcdn_url, $asset[0]);
        }

        // check if not a relative path
        if (!$this->relative || strstr($asset[0], $blog_url)) {
            return str_replace($subst_urls, $this->dcdn_url, $asset[0]);
        }

        // relative URL
        return $this->dcdn_url . $asset[0];
    }
     ...
     * @return  string  updated HTML doc with DCDN links
     */

    public function rewrite($html) {
        // check if HTTPS and use DCDN over HTTPS enabled
        ...
        // call the dcdn rewriter callback
        $dcdn_html = preg_replace_callback($regex_rule, [$this, 'rewrite_url'], $html);
